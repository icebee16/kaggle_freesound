...
def load_testdata(fname):
    ...
    def calc(fname_list):
        for idx in tqdm(range(len(fname_list))):
            # melspectrogram
            y, sr = read_audio(input_dir / "test" / fname_list[idx])
            spec_mono = audio_to_melspectrogram(y, sr)
            spec_color = mono_to_color(spec_mono)
            spec_list.append(spec_color)
...
class TestDataset(Dataset):
    ...
    def __init__(self, transform, tta=5):
        super().__init__()
        self.transforms = transform
        self.fnames = pd.read_csv(ROOT_PATH / "input" / "sample_submission.csv")["fname"].values.tolist()
...
def train_model(train_df, train_transforms):
    ...
    for epoch in mb:
        ...
        train_time += (time.time() - start) / num_epochs
        ...
        valid_time += (time.time() - start) / num_epochs

    get_logger().info("train time mean: {}\tvalid time mean: {}".format(train_time, valid_time - train_time))
